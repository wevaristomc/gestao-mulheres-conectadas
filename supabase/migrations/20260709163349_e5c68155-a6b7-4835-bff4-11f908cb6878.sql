...
CREATE POLICY "orbe_conversas_own"
  ON public.orbe_conversas FOR ALL TO authenticated
...
CREATE POLICY "orbe_mensagens_own"
  ON public.orbe_mensagens FOR ALL TO authenticated
...
-- 4. Registrar processo do Orbe em ia_politicas (se existir) -----------
DO $$
BEGIN
...
-- 5. Bônus de segurança: RLS + política "auth manage" em tabelas
--    hoje sem proteção. Só executa se a tabela existir.
DO $$
DECLARE
  t text;
  tabelas text[] := ARRAY[
    'projetos','pendencias','aulas','metas_indicadores','orcamento_itens',
    'cotacoes','cotacao_propostas','agent_runs','deq_chunks'
  ];
BEGIN
  FOREACH t IN ARRAY tabelas LOOP
    IF EXISTS (SELECT 1 FROM information_schema.tables
      ...
      EXECUTE format(
        'CREATE POLICY "auth users manage %I" ON public.%I FOR ALL TO authenticated USING (true) WITH CHECK (true);',
        t, t
      );
    END IF;
  END LOOP;

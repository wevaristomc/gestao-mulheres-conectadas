...
create policy polos_inscricao_coord_manage on public.polos_inscricao for all using (public.has_role_any(auth.uid(), array['coordenador_geral','coordenador_pedagogico','administrativo'])) with check (public.has_role_any(auth.uid(), array['coordenador_geral','coordenador_pedagogico','administrativo']));

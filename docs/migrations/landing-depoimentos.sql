-- Gestão dos depoimentos em vídeo da landing Mulheres Conectadas.
...
CREATE TRIGGER landing_depoimentos_atualizado_em
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.set_landing_depoimentos_atualizado_em();
...
CREATE POLICY landing_depoimentos_coord_select
  ON public.landing_depoimentos
  FOR SELECT
  TO authenticated
  USING (
    public.has_role_any(
...
CREATE POLICY landing_depoimentos_coord_insert
  ON public.landing_depoimentos
  FOR INSERT
  TO authenticated
  WITH CHECK (
...
CREATE POLICY landing_depoimentos_coord_update
  ON public.landing_depoimentos
  FOR UPDATE
  TO authenticated
  USING (
...
CREATE POLICY landing_depoimentos_coord_delete
  ON public.landing_depoimentos
  FOR DELETE
  TO authenticated
  USING (
...
VALUES ('landing', 'landing', true, 52428800, ARRAY['video/mp4'])
